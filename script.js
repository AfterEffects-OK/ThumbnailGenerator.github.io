(function() {
    ...
    // Webフォントを読み込む関数
    function loadWebFont(font, fontSize, fontFamily, selectElement) {
        if (font && font.url) {
...
fontFamilySelect1.addEventListener('change', (e) => {
    ...
    if (selectedFont) {
        ...
        loadWebFont(selectedFont, fontSize1, fontFamily1, fontFamilySelect1); // fontFamilySelect1 を渡す
    } else {
        fontFamily1 = 'Dela Gothic One';
        loadWebFont({ value: 'Dela Gothic One' }, fontSize1, fontFamily1, fontFamilySelect1); // fontFamilySelect1 を渡す
    }
    redrawCanvas();
});
...
fontFamilySelect2.addEventListener('change', (e) => {
    ...
    if (selectedFont) {
        ...
        loadWebFont(selectedFont, fontSize2, fontFamily2, fontFamilySelect2); // fontFamilySelect2 を渡す
    } else {
        fontFamily2 = 'Stick';
        loadWebFont({ value: 'Stick' }, fontSize2, fontFamily2, fontFamilySelect2); // fontFamilySelect2 を渡す
    }
    redrawCanvas();
});
    // JavaScript コード (ここまで)
